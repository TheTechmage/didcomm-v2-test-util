...
class BaseConnectionManager:
    ...
    async def create_did_peer_2(
        ...
        async with self._profile.session() as session:
            ...
            my_xk = Key.from_public_bytes(KeyAlg.ED25519, b58decode(verkey)).convert_key(KeyAlg.X25519)

            did = generate(
                [
                    KeySpec.verification(self._key_info_to_multikey(key)),
                    KeySpec.key_agreement(multibase.encode(
                        multicodec.wrap("x25519-pub", my_xk.get_public_bytes()), "base58btc"
                    ))
                ],
                services
            )
